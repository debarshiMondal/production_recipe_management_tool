...
def get_unit_cost():
    ...
    for file_path in [OUR_PRODUCTION_PRODUCTS_UNIT_COST, OUTSOURCED_PRODUCTS_UNIT_COST,RAW_MATERIAL_LIST]:
        if os.path.exists(file_path):
            df = pd.read_excel(file_path)
...
def get_raw_materials():
    raw_materials = []
    if os.path.exists('data/Packaging_Material_List.xlsx'):
...
def get_packaging_materials():
    ...
    return jsonify({'packagingMaterials': []})
